...
import { getOSMLyr } from "@/utils/openlayers/getLyr.js";
...
export default {
  ...
  mounted() {
    this.initMap();
    this.addLineWithPoints(this.routeCoords);
  },
  ...
  methods: {
    ...
    initMap() {
      this.map = new Map({
        ...
        view: new View({
          ...
          center: fromLonLat([117.0580924085071, 31.28721101756178]), //南京
          zoom: 10,
          maxZoom: 16,
          minZoom: 3,
        }),
    ...
    getStyle() {
      return new Style({
        ...
        image: new Icon({
          src: require("@/assets/demo/car.png"),
          rotateWithView: true,
          scale: 1,
        }),
      });
    ...
    addLineWithPoints(coordinates) {
      ...
      this.vectorLayer.getSource().addFeature(startFeature);
      //添加终点
      ...
      this.vectorLayer.getSource().addFeature(endFeature);
      ...
      // this.lineFeature.getGeometry().appendCoordinate(coordinates[0]);
      this.lineFeature.setStyle(lineStyle);
      carFeature.setStyle(carStyle);
      this.vectorLayer.getSource().addFeatures([carFeature, this.lineFeature]);
    ...
    moveFeature(event) {
      ...
      if (this.animating) {
        ...
        this.lineFeature
          .getGeometry()
          .appendCoordinate(this.routeCoords[index]);
      }
      console.info();
      this.map.render();
    },
